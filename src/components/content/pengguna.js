...
class Pengguna extends Component{
  ...
  handleSubmitDaftar(e){
    e.preventDefault();
    const {pageshow,fakultasc,jurusanc,nimc,namac,finger1,finger2} = this.state;
    const {filternim,namafilterc,kodematkulfilterc,kelasfilterc} = this.state;
    if (pageshow==='pengguna'){
      fetch(get.createpengguna, {
        method: 'post',
        headers :{
          "x-access-token" : sessionStorage.name,
          "Content-Type" : "application/json"
  ...
  render(){
    ...
    else if (state.pageshow==="filterpengguna"){
          ...
          <div className="kotakfilterpengguna">
            ...
            <a onClick={() => this.showFilterPengguna("filterpengguna",state.filternim)}>
              <div className="tombolfilterpengguna">
                ...
                <span><b>&nbsp;&nbsp;&nbsp;Mata Kuliah Pengguna</b></span>
              </div>
            </a>
          </div>
        </div>
            ...
            <table className="tablepengguna">
              <thead className="theadlog">
                <tr>
                  ...
                  <th className="nim" onClick={() => this.filter(state.page, "nim", state.ascdsc)}>NIM</th>
                    ...
                    <td>
                      <div>
                        <button className="backgroundbiru" onClick={() => this.showEdit(isidata.nim,isidata.nim,isidata.nama,'','')} >
                          ...
                          &nbsp;
                        <button className="backgroundmerah" onClick={() => this.deletePengguna(isidata.nim)}>
                          Delete
                        </button>
                      </div>
                    </td>
              ...
              <tbody className="tbodylog">
                {state.data.map(isidata => (
                  <tr key={i++}>
                    ...
                    <td>
                      <div>
                        <button className="backgroundmerah" onClick={() => this.deleteFilterPengguna(isidata.nim,isidata.nama,isidata.kodematkul,isidata.kelas)}>
                          Delete
                        </button>
                      </div>
                    </td>
